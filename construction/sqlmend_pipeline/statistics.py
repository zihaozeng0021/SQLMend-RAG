...
def _markdown_report(stats: dict[str, Any]) -> str:
    lines = [
        ...
        "| 指标 | 数值 |",
        "|---|---:|",
        f"| 原始文档 | {stats['raw_document_count']:,} |",
        f"| 清洗后文档 | {stats['cleaned_document_count']:,} |",
        f"| 最终 chunks | {stats['final_chunk_count']:,} |",
        f"| 总词数 | {stats['total_word_count']:,} |",
        f"| 近似唯一词数 | {stats['approximate_unique_word_count']:,} |",
        f"| 平均 chunk 词数 | {stats['average_chunk_word_count']:.2f} |",
        f"| 中位 chunk 词数 | {stats['median_chunk_word_count']:.2f} |",
        f"| 最短 / 最长 | {stats['minimum_chunk_word_count']} / {stats['maximum_chunk_word_count']} |",
        f"| 版本信息已知 | {stats['version_known_percentage']:.2f}% |",
        f"| SQL 相关 chunk | {stats['sql_chunk_percentage']:.2f}% |",
        f"| 错误信息相关 chunk | {stats['error_chunk_percentage']:.2f}% |",
        f"| 版本/兼容性相关 chunk | {stats['version_or_compatibility_chunk_percentage']:.2f}% |",
        "",
        "## 方言分布",
        "",
        "| 方言 | 文档 | chunks | 占比 |",
    ...
    lines.extend(
        [
            ...
            "## 去重与采集异常",
            "",
            f"- 文档级和 chunk 级一共删除精确重复 {stats['exact_duplicate_count_removed']:,} 条。",
            f"- 一共删除近似重复 {stats['near_duplicate_count_removed']:,} 条。",
            f"- 失败来源 {stats['failed_source_count']} 个，失败 URL {stats['failed_url_count']} 个，其中不可访问来源 {stats['inaccessible_source_count']} 个。",
            "",
