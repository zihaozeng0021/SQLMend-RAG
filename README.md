...
这个仓库做的是 SQLMendRAG 的“资料准备”部分：把五种数据库的官方资料抓下来，清洗、补元数据、去重、切块，最后得到可以直接拿去做 BM25 或向量索引的 `corpus.jsonl`。这里**没有**实现回答生成器，也不会把后续人工标注数据混进来。
...
| 自动验证 / 测试 | 24/24 PASS；86 tests PASS |
