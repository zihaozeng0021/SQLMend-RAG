...
本目录实现阶段 5-6 的三套独立、可审计基线：BM25 稀疏检索、零样本 E5 稠密检索，以及只融合前两者排名的 RRF hybrid。流水线自行控制语料加载、schema 验证、查询序列化、索引、精确检索、融合、TREC 导出、pool 审计、评估、性能记录和验证；第三方库只提供算法组件，不会把语料或查询发送给托管检索/RAG 服务。
